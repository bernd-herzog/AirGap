/*
 * Meine Testimplementation für die Fehlerkorrektur
 * Grundlage: https://en.wikiversity.org/wiki/Reed%E2%80%93Solomon_codes_for_coders
 * 
 * Unter Windows/Visualstudio bekomme ich noch einige Fehlermeldungen
 * Ich glaube weil VS nicht C99 100%ig umsetzt und mit dem direkten Übergeben von Arrays (also nicht in Pointer schreibweise) nicht ganz einverstanden ist
 * 
 * Ich hoffe mal es ist nicht zu ineffizent/langsam.
 * 
 * Linux/GCC - Kommando:
 * gcc test_reedsolomon.c -std=c99 -o test_reedsolomon
 * 
 * Markus 
 * */
...
void rs_encode_msg(unsigned char chunk[], unsigned char parity[], int size)
{
	unsigned char gen[nsym+1];
	rs_generator_poly(gen);
	
	unsigned char msg_out[size+nsym];
	for(int x=0; x<size;x++)
	{
		msg_out[x]=chunk[x];
	}
	for(int y=size; y<size+nsym;y++)
	{
		msg_out[y]=0;
	}
...
int rs_find_errors(unsigned char err_pos[], unsigned char synd[], int nmess, int *err_pos_size) //Berlekamp-Massey algorithmus
{	
	...
	for(int x=0;x<nsym;x++)
	{
		...
		if(delta != 0)
		{
			...
			unsigned char tmp_poly[old_poly_size];
			...
			unsigned char tmp_error_poly[tmp_error_poly_size];
...
void rs_correct_errata(unsigned char msg[], unsigned char synd[], unsigned char err_pos[], int size, int err_pos_size)
{
	...
	unsigned char r[err_pos_size];
	for(int i=0;i<err_pos_size;i++)
	{
		r[i]=p[p_size-err_pos_size+i];
	}
	unsigned char s[q_size/2];
	int s_size=0;
	if(q_size>1){
		if(q_size % 2)
		{
...
void rs_correct_msg(unsigned char msg[], int size)
{
	...
	rs_calc_syndromes(msg_out, synd, size);
	...
	int err_pos_size=0;
	rs_find_errors(err_pos, synd, size,&err_pos_size);
	
	if(err_pos_size==0)
	{
		return;//keine Fehler gefunden
	}
	rs_correct_errata(msg_out, synd, err_pos, size, err_pos_size);
	rs_calc_syndromes(msg_out, synd, size);
	for(int x=0; x<size;x++)
	{
		msg[x]=msg_out[x];		
